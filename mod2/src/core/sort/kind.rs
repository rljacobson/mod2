...
impl Kind {
  ...
  pub fn new(mut initial_sort: SortPtr) -> Result<BxKind, KindError> {
    ...
    */
    ...
    unsafe{ kind.register_connected_sorts(initial_sort, &mut visited_sort_count); }
    ...
    // Process subsorts. Length of `kind.sorts` may increase.
    {
      ...
      loop {
        ...
        unsafe{ (*kind).process_subsorts((*kind).sorts[i]); }
        i += 1
      }
    }
    ...
    for i in (0..visited_sort_count).rev() {
      unsafe{ (*kind.sorts[i as usize]).compute_leq_sorts(); }
    }
  ...
  fn register_connected_sorts(&mut self, sort: SortPtr, visited_sort_count: &mut u32) {
    unsafe{ (*sort).kind = self; }
    *visited_sort_count += 1;

    unsafe{ // Visit subsorts
...
impl Display for Kind {
  fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
    let iter = self.sorts.iter().map(|s_ptr| unsafe{ (**s_ptr).name.as_ref() });
