...
use crate::api::automaton::{BxLHSAutomaton, RHSAutomaton};
use crate::api::free_theory::{FreeOccurrence, FreeOccurrences, FreeTerm};
use crate::api::term::{Term, TermPtr};
use crate::api::variable_theory::{VariableIndex, VariableTerm};
...
impl FreeTerm {
  fn scan_free_skeleton(
    ...
    let oc = FreeOccurrence::new(parent, arg_index, self.as_ptr());
    free_symbols.push(oc);

    for (i, mut term) in self.args.iter().copied().enumerate() {
      if let Some(free_term) = term.deref_mut().as_any_mut().downcast_mut::<FreeTerm>() {
        free_term.scan_free_skeleton(free_symbols, other_symbols, our_position, i as i32);
      } else {
        let oc2 = FreeOccurrence::new(our_position, i as i32, term.as_ptr());
  ...
  pub fn compile_lhs(
    ...
    for mut occurrence in other_symbols {
      if let Some(v) = occurrence.try_downcast_term_mut::<VariableTerm>() {
        ...
        let term: &mut dyn Term = occurrence.term_mut();
        if term.ground() {
...
*/
  ...
  pub fn compile_remainder(&self, equation: &PreEquation, slot_translation: &Vec<i32>) -> FreeRemainder {
    ...
    let mut best_sequence = ConstraintPropagationSequence::default();
    let mut sub_automata: Vec<BxLHSAutomaton> = Vec::new();
  ...
  pub fn analyse_constraint_propagation(&mut self, bound_uniquely: &mut NatSet) {
    ...
    for occurrence in other_symbols.iter_mut() {
    ...
    for i in 0..arg_count as usize {
      ...
      if let Some(free_term) = term.deref_mut().as_any_mut().downcast_mut::<FreeTerm>() {
        *free_variable_count += 1;
        if !available_terms.contains(*term, arg_eager) {
