...
A sort (represented in code by the [`Sort`] struct) is a named type.
...
context, which are called *kinds* (represented by the [`Kind`](kind::Kind)
...
`Sort`s are created as part of their parent `Module`.
...
responsibility of destroying them (and freeing their memory).

## The Subsort Relation

Any two sorts are either incomparable, or equal, or one sort is a subsort of the other. The
subsort relation is represented by adjacency lists stored in the sorts themselves: each sort
stores a list of (pointers to) its subsorts and a list of (pointers to) its supersorts.
...
precomputation performed during the construction of the `Kind`s and involves the
implementation of both `Kind` and `Sort`. We describe this precomputation here.

### Sort Indexing and the Subsort Relation

The `index_within_kind` property of `Sort` provides a numeric index that helps to
efficiently navigate the sort hierarchy. The key principle is that if `x.index_within_kind
> y.index_within_kind`, then `x` is never a supersort of `y`. This ordering facilitates
an optimization: the value `Sort.fast_compare_index` is set to the smallest
`index_within_kind` such that `x.index_within_kind >= fast_compare_index` ensures `x`
is a subsort of `y`. When this comparison can be made, the adjacency lists do not
need to be searched, significantly speeding up the comparison operation. For sort
heirarchies that are "nearly" or completely linear, this can be a significant savings.

### Role of `Sort.unresolved_supersort_count`

The `unresolved_supersort_count` property of `Sort` acts as a counter for the number of
supersorts of a given sort that have yet to be explored or registered within their `Kind`
(connected component). This property ensures that a sort's `index_within_kind` is only
...
as there may be that some incomporable `Sort` that has a lower `index_within_kind`.)

### Depth-First Search in Registering Sorts

The methods `Kind::register_connected_sorts` and `Sort::process_subsorts` implement a depth-first
search through the sort lattice. This approach guarantees that each sort *and its subsorts* are
fully explored and registered in the connected component before moving to the next sort. This
systematic traversal is essential for correctly establishing the `index_within_kind` for each
`Sort` and for maintaining the subsort-supersort relationships accurately within each `Kind`.

### The `Sort::compute_leq_sorts` Method and `fast_compare_index` Calculation

The `Sort::compute_leq_sorts` method is used to build a `leq_sorts` set, which includes all
sorts that are less than or equal to a given sort, based on the `index_within_kind`. The
`fast_compare_index` is an optimization that identifies the smallest `index_within_kind` for which
any sort with an index greater or equal is guaranteed to be less than or equal to the current sort.
The calculation of `fast_compare_index` cannot simply assume it to be equal to `index_within_kind`,
...
2. if `A.index_within_kind >= B.fast_compare_index`, then `A` is a subsort of `B`;
   if `B.index_within_kind >= A.fast_compare_index`, then `B` is a subsort of `A`;
